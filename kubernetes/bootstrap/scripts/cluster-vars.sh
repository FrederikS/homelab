...
function apply_config_maps() {
    ...
    for config in "${configs[@]}"; do
        if [ ! -f "${config}" ]; then
            log warn "File does not exist" "file=${config}"
            continue
        fi

        # Check if the config resources are up-to-date
        if kubectl --namespace flux-system diff --filename ${config} &>/dev/null; then
            log info "Config resource is up-to-date" "resource=$(basename "${config}" ".yaml")"
            continue
        fi

        # Apply config resources
        if kubectl --namespace flux-system apply --server-side --filename ${config} &>/dev/null; then
            log info "Config resource applied successfully" "resource=$(basename "${config}" ".yaml")"
        else
            log error "Failed to apply config resource" "resource=$(basename "${config}" ".yaml")"
        fi
    done
}
...
#         https://raw.githubusercontent.com/kubernetes-sigs/external-dns/refs/tags/v0.21.0/config/crd/standard/dnsendpoint.yaml
#     )
...
function main() {
    check_cli kubectl kustomize sops
